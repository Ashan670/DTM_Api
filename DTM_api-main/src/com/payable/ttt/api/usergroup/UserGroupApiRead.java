...
/*
 * @author  Ashan prmodya
 * @version 1.0
 * @since   2024-06-7
 */

public abstract class UserGroupApiRead extends TeamTimeTrackerBaseHandler {
    private static final long serialVersionUID = 2211240845L;

    protected BaseDbManager<ORMUser> dbm_CorpUser;
    protected BaseDbManager<ORMUserGroup> dbm_CorpUserGroup;

    public UserGroupApiRead() {
        super();
        dbm_CorpUserGroup = new BaseDbManager<>(hibernateFactory);
        dbm_CorpUser = new BaseDbManager<>(hibernateFactory);
    }
    ...
    protected String processRequest(HttpServletRequest request, HttpServletResponse response, ORMUser user,
        ...
        return processUserGroup(csr, userId, ipAddress);
    }

    protected abstract String processUserGroup(CommonSearchRequest csr, String userId, String ipAddress)

...
@WebServlet("/report/teamReport")
public class TeamReport extends ReadDataAPI {
	...
	protected String processReportDataFetch(CommonSearchRequest csr, String userId, String ipAddress)
		...
		if (userId.equalsIgnoreCase(SysConfigParam.STAFF_UUID)) {
			log.error("(2211241152)Invalid page Id : " + csr.getPageId());
		...
		// String hqltaskDetail = " FROM ORMUser tbl where tbl.id = '" + userId + "'";
		...
		String grpId = taskDetail.get(0).getUserGroup().getId();

		String hqlUserRoles = " FROM ORMReportBasickDetail tbl " + finalWhere + " AND user_group_id='" + grpId + "'  ORDER BY  firstName,  startTimeEnterd DESC ";
		...
		if (csr.getStDate() == null) {
			listUserRole = dbm_RreportBasic.getRecords(hqlUserRoles, csr.getPageSize() * csr.getPageId(),
					csr.getPageSize());
		} else {
			try {
				listUserRole = dbm_RreportBasic.getRecords(hqlUserRoles, csr.getPageSize() * csr.getPageId(),
		...
		String formatedTotalDuration = "-1";
		if (csr.getPageId() == 0) {
			String hqlCount = "SELECT count(*) FROM ORMReportBasickDetail tbl  " + finalWhere + "  AND user_group_id='"	+ grpId + "'";
